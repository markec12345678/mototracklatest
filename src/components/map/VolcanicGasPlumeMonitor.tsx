...
import { useMapStore, type VolcanicGasPlumeState } from '@/lib/map-store'
import { Flame as FlameIcon12, X, MapPin, TrendingUp, TrendingDown, Minus, Activity } from 'lucide-react'

const SAMPLE_LOCATIONS = [
  { name: 'Kilauea', value: 4500, unit: 't/day', status: 'critical', trend: 'up', lat: 19.4, lng: -155.3, description: 'Persistent SO₂ plume from active lava lake and effusive eruption' },
  { name: 'Holuhraun Iceland', value: 2800, unit: 't/day', status: 'warning', trend: 'stable', lat: 64.9, lng: -18.6, description: 'Fissure eruption gas emissions with aviation hazard potential' },
  { name: 'Ambrym Vanuatu', value: 3200, unit: 't/day', status: 'critical', trend: 'up', lat: -16.3, lng: 168.1, description: 'Lava lake degassing with persistent volcanic gas plume' },
  { name: 'Popocatepetl Mexico', value: 1800, unit: 't/day', status: 'warning', trend: 'down', lat: 19.0, lng: -98.6, description: 'Periodic explosive emissions with dome growth and destruction' },
]
...
export function VolcanicGasPlumeMonitor() {
  const data = useMapStore((s) => s.volcanicGasPlume.data)
  const setData = useMapStore((s) => s.setVolcanicGasPlume)

  useEffect(() => {
    if (!data) {
      setData({
        data: {
          name: 'Global Volcanic Gas Plume Monitor',
          value: 3075.0,
          unit: 'SO₂ flux (t/day)',
          status: 'warning',
          trend: 'up',
          lat: 19.4,
          lng: -155.3,
          description: 'Monitoring volcanic SO₂ gas plume emissions and flux at active volcanoes worldwide',
          ...
          <CardHeader className="pb-3 border-b border-orange-800/30">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2 text-orange-100">
                <FlameIcon12 className="h-4 w-4 text-orange-400" />
                Volcanic Gas Plume Monitor
              </CardTitle>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-orange-700/30 bg-orange-900/30 p-2 text-center">
                    <div className="text-[10px] text-orange-400">SO₂ Flux</div>
