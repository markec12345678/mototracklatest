...
import { useMapStore, type TidalWhirlpoolState } from '@/lib/map-store'
import { Waves as WavesIcon13, X, MapPin, TrendingUp, TrendingDown, Minus, Activity } from 'lucide-react'

const SAMPLE_LOCATIONS = [
  { name: 'Saltstraumen Norway', value: 22, unit: 'vortex intensity (knots)', status: 'critical', trend: 'up', lat: 67.2, lng: 14.6, description: 'World\'s strongest tidal current, maelstrom formation every tide cycle' },
  { name: 'Moskstraumen Norway', value: 18, unit: 'vortex intensity (knots)', status: 'warning', trend: 'up', lat: 67.9, lng: 12.7, description: 'Legendary maelstrom, powerful tidal vortex system between islands' },
  { name: 'Corryvreckan Scotland', value: 16, unit: 'vortex intensity (knots)', status: 'warning', trend: 'stable', lat: 56.2, lng: -5.7, description: 'Third largest whirlpool globally, dangerous tidal race in the Inner Hebrides' },
  { name: 'Naruto Japan', value: 12, unit: 'vortex intensity (knots)', status: 'normal', trend: 'down', lat: 34.2, lng: 134.7, description: 'Famous tidal whirlpools under the Onaruto Bridge, seasonal variation' },
]
...
export function TidalWhirlpoolMonitor() {
  const data = useMapStore((s) => s.tidalWhirlpool.data)
  const setData = useMapStore((s) => s.setTidalWhirlpool)

  useEffect(() => {
    if (!data) {
      setData({
        data: {
          name: 'Global Tidal Whirlpool Monitor',
          value: 17.0,
          unit: 'avg vortex intensity (knots)',
          status: 'warning',
          trend: 'up',
          lat: 67.2,
          lng: 14.6,
          description: 'Monitoring tidal whirlpool and maelstrom intensity at major vortex sites worldwide',
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-blue-700/30 bg-blue-900/30 p-2 text-center">
                    <div className="text-[10px] text-blue-400">Avg Vortex Intensity</div>
