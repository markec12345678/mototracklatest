...
import { Waves as WavesIcon26, X, MapPin, Filter } from 'lucide-react'
...
const SAMPLE_LOCATIONS: MonitorData[] = [
  {
    ...
    recoveryPct: 45,
    status: 'warning',
    description: 'Large MSF thermal desalination complex co-located with power generation plant',
  },
  {
    id: 'ds-sorek',
    name: 'Sorek Israel',
    ...
    recoveryPct: 52,
    status: 'stable',
    description: 'World largest SWRO reverse osmosis plant with advanced energy recovery devices',
  },
  {
    id: 'ds-carlsbad',
    name: 'Carlsbad CA',
    ...
    recoveryPct: 50,
    status: 'stable',
    description: 'Reverse osmosis plant supplying San Diego County with drought-resilient water',
  },
  {
    id: 'ds-sydneydesal',
    name: 'Sydney Desal',
    ...
    recoveryPct: 47,
    status: 'moderate',
    description: 'Coastal SWRO plant with renewable energy offset for Sydney metropolitan supply',
  },
]
...
export function DesalinationOutputMonitor() {
  const state = useMapStore((s) => s.desalinationOutput)
  const setState = useMapStore((s) => s.setDesalinationOutput)
  ...
  const summary = useMemo(() => {
    if (filteredItems.length === 0) {
      return { totalOutput: 0, avgEnergy: 0, avgSalinity: 0, avgRecovery: 0 }
    }
    const totalOutput = filteredItems.reduce((sum, e) => sum + (e.outputMLday as number), 0)
    const avgEnergy = filteredItems.reduce((sum, e) => sum + (e.energyKwhM3 as number), 0) / filteredItems.length
    const avgSalinity = filteredItems.reduce((sum, e) => sum + (e.salinityPpm as number), 0) / filteredItems.length
  ...
  useEffect(() => {
    if (state.data.length === 0) {
      useMapStore.getState().setDesalinationOutput({ data: SAMPLE_LOCATIONS })
    }
  }, [state.data.length])
        ...
        <CardHeader className="pb-3 border-b border-slate-700/30">
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm flex items-center gap-2 text-slate-100">
              <WavesIcon26 className="h-4 w-4 text-sky-200" />
        ...
        <CardContent className="space-y-3 p-4 text-slate-100">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-lg border border-slate-700/30 bg-slate-900/30 p-2 text-center">
              ...
              <div className="text-sm font-semibold text-sky-200">{summary.totalOutput}</div>
              <div className="text-[9px] text-slate-400/60">ML/day total</div>
            </div>
            <div className="rounded-lg border border-slate-700/30 bg-slate-900/30 p-2 text-center">
              <div className="text-[10px] text-slate-400/70">Energy</div>
              <div className="text-sm font-semibold text-blue-200">{summary.avgEnergy}</div>
              <div className="text-[9px] text-slate-400/60">kWh/m3 avg</div>
            </div>
            <div className="rounded-lg border border-slate-700/30 bg-slate-900/30 p-2 text-center">
              <div className="text-[10px] text-slate-400/70">Salinity</div>
              <div className="text-sm font-semibold text-cyan-200">{summary.avgSalinity}</div>
            ...
            <div className="rounded-lg border border-slate-700/30 bg-slate-900/30 p-2 text-center">
              ...
              <div className="text-[9px] text-slate-400/60">avg efficiency</div>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <Label className="text-xs text-slate-300/80">
              Desalination Plants ({filteredItems.length})
            </Label>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-[10px] text-slate-300/60">
                        ...
                        <div>
                          Energy: <span className="text-slate-100 font-medium">{e.energyKwhM3} kWh/m3</span>
                        </div>
                        <div>
                          Salinity: <span className="text-slate-100 font-medium">{(e.salinityPpm as number).toLocaleString()} ppm</span>
                        </div>
                        <div>
              ...
              <div className="space-y-2 rounded-lg border border-slate-600/30 bg-slate-800/20 p-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  ...
                  <div>
                    ...
                    <span className="font-medium text-sky-200">{activeItem.outputMLday as number} ML/day</span>
                  </div>
                </div>
              </div>
            </>
