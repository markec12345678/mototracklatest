...
import { useMapStore, type MeteorImpactState } from '@/lib/map-store'
import { Zap as ZapIcon3, X, MapPin, TrendingUp, TrendingDown, Minus, Activity } from 'lucide-react'

const SAMPLE_LOCATIONS = [
  { name: 'Barringer Crater, Arizona', value: 94, unit: 'impact score', status: 'normal', trend: 'stable', lat: 35.027, lng: -111.023, description: '1.2 km diameter meteor crater formed ~50,000 years ago' },
  { name: 'Chicxulub, Yucatán', value: 100, unit: 'impact score', status: 'normal', trend: 'stable', lat: 21.395, lng: -89.317, description: 'Dinosaur-killing impact crater, 180 km diameter' },
  { name: 'Tunguska Event Site', value: 72, unit: 'impact score', status: 'normal', trend: 'stable', lat: 60.886, lng: 101.894, description: '1908 airburst event that flattened 2,000 km² of forest' },
  { name: 'Chelyabinsk Impact Zone', value: 58, unit: 'impact score', status: 'warning', trend: 'up', lat: 55.154, lng: 61.385, description: '2013 meteor airburst, largest in over a century' },
]
...
function TrendIcon({ trend }: { trend: string }) {
  if (trend === 'up') return <TrendingUp className="h-3 w-3 text-amber-400" />
  if (trend === 'down') return <TrendingDown className="h-3 w-3 text-emerald-400" />
  return <Minus className="h-3 w-3 text-slate-400" />
}

export function MeteorImpactMonitor() {
  const data = useMapStore((s) => s.meteorImpact.data)
  const setData = useMapStore((s) => s.setMeteorImpact)

  useEffect(() => {
    if (!data) {
      setData({
        data: {
          name: 'Meteor Impact Tracker',
          value: 81.0,
          unit: 'avg impact score',
          status: 'normal',
          trend: 'stable',
          lat: 35.027,
          lng: -111.023,
          description: 'Tracking meteor impact sites and near-Earth object events worldwide',
          ...
          <CardHeader className="pb-3 border-b border-purple-800/30">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2 text-purple-100">
                <ZapIcon3 className="h-4 w-4 text-purple-400" />
                Meteor Impact Tracker
              </CardTitle>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-purple-700/30 bg-purple-900/30 p-2 text-center">
                    <div className="text-[10px] text-purple-400">Impact Score</div>
                        ...
                        <div key={loc.name} className="rounded-lg border border-purple-800/30 p-2.5 hover:bg-purple-900/20 transition-all">
                          <div className="flex items-center justify-between mb-1.5">
                            <div className="flex items-center gap-1.5">
                              <div className="h-2 w-2 rounded-full" style={{ backgroundColor: loc.status === 'critical' ? '#ef4444' : loc.status === 'warning' ? '#f59e0b' : '#a855f7' }} />
