...
import { useMapStore, type DesertPavementState } from '@/lib/map-store'
import { Sun as SunIcon9, X, MapPin, TrendingUp, TrendingDown, Minus, Activity } from 'lucide-react'

const SAMPLE_LOCATIONS = [
  { name: 'Mojave Desert', value: 72, unit: '%', status: 'warning', trend: 'down', lat: 35.0, lng: -116.0, description: 'Desert pavement disturbance from off-road vehicles and military activity' },
  { name: 'Atacama Desert', value: 88, unit: '%', status: 'normal', trend: 'stable', lat: -24.0, lng: -70.0, description: 'Well-preserved pavement surface in hyper-arid conditions' },
  { name: 'Negev Desert', value: 65, unit: '%', status: 'warning', trend: 'down', lat: 31.0, lng: 34.0, description: 'Pavement degradation from agricultural expansion and grazing' },
  { name: 'Taklamakan', value: 58, unit: '%', status: 'critical', trend: 'down', lat: 39.0, lng: 82.0, description: 'Severe surface disruption from sand encroachment and dust storms' },
]
...
export function DesertPavementMonitor() {
  const data = useMapStore((s) => s.desertPavement.data)
  const setData = useMapStore((s) => s.setDesertPavement)

  useEffect(() => {
    if (!data) {
      setData({
        data: {
          name: 'Global Desert Pavement Monitor',
          value: 70.8,
          unit: 'surface integrity (%)',
          status: 'warning',
          trend: 'down',
          lat: 35.0,
          lng: -116.0,
          description: 'Monitoring desert pavement surface integrity and disturbance at arid regions worldwide',
          ...
          <CardHeader className="pb-3 border-b border-yellow-800/30">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2 text-yellow-100">
                <SunIcon9 className="h-4 w-4 text-yellow-400" />
                Desert Pavement Monitor
              </CardTitle>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-yellow-700/30 bg-yellow-900/30 p-2 text-center">
                    <div className="text-[10px] text-yellow-400">Surface Integrity</div>
