...
import { useMapStore, type GlacierLakeState } from '@/lib/map-store'
import { Droplets as DropletsIcon12, X, MapPin, TrendingUp, TrendingDown, Minus, Activity } from 'lucide-react'

const SAMPLE_LOCATIONS = [
  { name: 'Imja Lake, Nepal', value: 1.28, unit: 'km²', status: 'critical', trend: 'up', lat: 27.9, lng: 87, description: 'Rapidly expanding glacial lake posing GLOF risk to downstream communities' },
  { name: 'Merzbacher Lake, Kyrgyzstan', value: 0.85, unit: 'km²', status: 'warning', trend: 'stable', lat: 42, lng: 80, description: 'Ice-dammed lake with periodic outburst floods' },
  { name: 'Palcacocha, Peru', value: 0.72, unit: 'km²', status: 'critical', trend: 'up', lat: -9.4, lng: -77, description: 'High-risk glacial lake above Huaraz city' },
  { name: 'Thorthormi, Bhutan', value: 0.95, unit: 'km²', status: 'warning', trend: 'up', lat: 27.8, lng: 89.8, description: 'Growing lake with potential cascading GLOF hazard' },
]
...
export function GlacierLakeMonitor() {
  const data = useMapStore((s) => s.glacierLake.data)
  const setData = useMapStore((s) => s.setGlacierLake)

  useEffect(() => {
    if (!data) {
      setData({
        data: {
          name: 'Global Glacier Lake Monitor',
          value: 0.95,
          unit: 'km²',
          status: 'warning',
          trend: 'up',
          lat: 27.9,
          lng: 87,
          description: 'Monitoring glacial lake expansion and GLOF risk worldwide',
          ...
          <CardHeader className="pb-3 border-b border-blue-800/30">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2 text-blue-100">
                <DropletsIcon12 className="h-4 w-4 text-blue-400" />
                Glacier Lake Monitor
              </CardTitle>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-blue-700/30 bg-blue-900/30 p-2 text-center">
                    <div className="text-[10px] text-blue-400">Avg Lake Area</div>
                    <div className="text-lg font-semibold text-blue-200">{data.value.toFixed(2)}</div>
