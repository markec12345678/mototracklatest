...
import { useMapStore, type VolcanicLaharState } from '@/lib/map-store'
import { Mountain as MountainIcon9, X, MapPin, TrendingUp, TrendingDown, Minus, Activity } from 'lucide-react'

const SAMPLE_LOCATIONS = [
  { name: 'Mt. Pinatubo Lahar Zone', value: 78, unit: 'flow index', status: 'critical', trend: 'up', lat: 15.142, lng: 120.349, description: 'Active lahar flow zone from 1991 eruption, heavy rainfall triggers mudflows' },
  { name: 'Mt. Rainier Lahar Path', value: 65, unit: 'flow index', status: 'warning', trend: 'up', lat: 46.852, lng: -121.760, description: 'Potential lahar path through river valleys, glacier melt risk' },
  { name: 'Cotopaxi Lahar Channel', value: 52, unit: 'flow index', status: 'warning', trend: 'stable', lat: -0.677, lng: -78.437, description: 'Active volcanic lahars through drainage channels' },
  { name: 'Ruapehu Crater Lake', value: 40, unit: 'flow index', status: 'normal', trend: 'down', lat: -39.281, lng: 175.569, description: 'Crater lake lahar risk during eruptions and breakout floods' },
]
...
export function VolcanicLaharMonitor() {
  const data = useMapStore((s) => s.volcanicLahar.data)
  const setData = useMapStore((s) => s.setVolcanicLahar)

  useEffect(() => {
    if (!data) {
      setData({
        data: {
          name: 'Global Volcanic Lahar Monitor',
          value: 58.8,
          unit: 'avg flow index',
          status: 'warning',
          trend: 'up',
          lat: 15.142,
          lng: 120.349,
          description: 'Monitoring volcanic lahar and mudflow events at active volcanoes worldwide',
          ...
          <CardHeader className="pb-3 border-b border-orange-800/30">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2 text-orange-100">
                <MountainIcon9 className="h-4 w-4 text-orange-400" />
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-orange-700/30 bg-orange-900/30 p-2 text-center">
                    <div className="text-[10px] text-orange-400">Avg Flow Index</div>
