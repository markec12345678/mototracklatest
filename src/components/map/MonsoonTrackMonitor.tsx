...
import { useMapStore, type MonsoonTrackState } from '@/lib/map-store'
import { CloudRain as CloudRainIcon5, X, MapPin, TrendingUp, TrendingDown, Minus, Activity } from 'lucide-react'

const SAMPLE_LOCATIONS = [
  { name: 'Indian Subcontinent', value: 12.5, unit: 'rainfall anomaly (mm/day)', status: 'critical', trend: 'up', lat: 22, lng: 80, description: 'Extreme monsoon rainfall exceeding historical averages by significant margin' },
  { name: 'Southeast Asia', value: 8.3, unit: 'rainfall anomaly (mm/day)', status: 'warning', trend: 'up', lat: 15, lng: 105, description: 'Above-normal monsoon precipitation affecting low-lying regions' },
  { name: 'West Africa', value: 5.1, unit: 'rainfall anomaly (mm/day)', status: 'warning', trend: 'stable', lat: 10, lng: -5, description: 'Variable West African monsoon with localized flooding patterns' },
  { name: 'South America', value: -2.4, unit: 'rainfall anomaly (mm/day)', status: 'normal', trend: 'down', lat: -5, lng: -60, description: 'Below-average monsoon season in Amazon basin region' },
]
...
export function MonsoonTrackMonitor() {
  const data = useMapStore((s) => s.monsoonTrack.data)
  const setData = useMapStore((s) => s.setMonsoonTrack)

  useEffect(() => {
    if (!data) {
      setData({
        data: {
          name: 'Global Monsoon Track Monitor',
          value: 5.88,
          unit: 'avg rainfall anomaly (mm/day)',
          status: 'warning',
          trend: 'up',
          lat: 22,
          lng: 80,
          description: 'Tracking monsoon rainfall anomalies and trajectory shifts across global monsoon regions',
          ...
          <CardHeader className="pb-3 border-b border-emerald-800/30">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2 text-emerald-100">
                <CloudRainIcon5 className="h-4 w-4 text-emerald-400" />
                Monsoon Track Monitor
              </CardTitle>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-emerald-700/30 bg-emerald-900/30 p-2 text-center">
                    <div className="text-[10px] text-emerald-400">Avg Rainfall Anomaly</div>
