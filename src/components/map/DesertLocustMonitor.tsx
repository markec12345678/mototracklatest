...
import { useMapStore, type DesertLocustState } from '@/lib/map-store'
import { Bug as BugIcon4, X, MapPin, TrendingUp, TrendingDown, Minus, Activity } from 'lucide-react'

const SAMPLE_LOCATIONS = [
  { name: 'Horn of Africa', value: 85, unit: 'per km²', status: 'critical', trend: 'up', lat: 10, lng: 43, description: 'Major breeding ground with severe swarm activity during rainy season' },
  { name: 'Sahel Region', value: 65, unit: 'per km²', status: 'warning', trend: 'stable', lat: 14, lng: -8, description: 'Seasonal locust breeding zone with monitoring stations active' },
  { name: 'Arabian Peninsula', value: 55, unit: 'per km²', status: 'warning', trend: 'up', lat: 20, lng: 47, description: 'Cross-border swarm migration corridor' },
  { name: 'Indo-Pakistan Border', value: 35, unit: 'per km²', status: 'normal', trend: 'down', lat: 27, lng: 68, description: 'Summer breeding area under surveillance' },
]
...
export function DesertLocustMonitor() {
  const data = useMapStore((s) => s.desertLocust.data)
  const setData = useMapStore((s) => s.setDesertLocust)

  useEffect(() => {
    if (!data) {
      setData({
        data: {
          name: 'Global Desert Locust Monitor',
          value: 60,
          unit: 'per km²',
          status: 'warning',
          trend: 'up',
          lat: 10,
          lng: 43,
          description: 'Tracking desert locust swarm formations and migration patterns',
          ...
          <CardHeader className="pb-3 border-b border-amber-800/30">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2 text-amber-100">
                <BugIcon4 className="h-4 w-4 text-amber-400" />
                Desert Locust Monitor
              </CardTitle>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-amber-700/30 bg-amber-900/30 p-2 text-center">
                    <div className="text-[10px] text-amber-400">Avg Swarm Density</div>
