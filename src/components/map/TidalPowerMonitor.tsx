...
import { useMapStore, type TidalPowerState } from '@/lib/map-store'
import { Zap as ZapIcon5, X, MapPin, TrendingUp, TrendingDown, Minus, Activity } from 'lucide-react'

const SAMPLE_LOCATIONS = [
  { name: 'Bay of Fundy', value: 7000, unit: 'MW', status: 'normal', trend: 'stable', lat: 45, lng: -65, description: 'Highest tidal range in the world, massive power generation potential' },
  { name: 'Pentland Firth', value: 3800, unit: 'MW', status: 'normal', trend: 'up', lat: 58.7, lng: -3.5, description: 'UK tidal stream energy hub with strong currents' },
  { name: 'La Rance, France', value: 240, unit: 'MW', status: 'normal', trend: 'stable', lat: 48.6, lng: -2, description: 'World first tidal power station, operational since 1966' },
  { name: 'Uldolmok, Korea', value: 450, unit: 'MW', status: 'warning', trend: 'up', lat: 34.5, lng: 126.5, description: 'Fast tidal currents with demonstration tidal stream project' },
]
...
export function TidalPowerMonitor() {
  const data = useMapStore((s) => s.tidalPower.data)
  const setData = useMapStore((s) => s.setTidalPower)

  useEffect(() => {
    if (!data) {
      setData({
        data: {
          name: 'Global Tidal Power Monitor',
          value: 2872.5,
          unit: 'MW',
          status: 'normal',
          trend: 'up',
          lat: 45,
          lng: -65,
          description: 'Monitoring tidal energy potential and generation sites worldwide',
          ...
          <CardHeader className="pb-3 border-b border-emerald-800/30">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2 text-emerald-100">
                <ZapIcon5 className="h-4 w-4 text-emerald-400" />
                Tidal Power Monitor
              </CardTitle>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-emerald-700/30 bg-emerald-900/30 p-2 text-center">
                    <div className="text-[10px] text-emerald-400">Avg Potential Power</div>
                    <div className="text-lg font-semibold text-emerald-200">{data.value.toFixed(0)}</div>
