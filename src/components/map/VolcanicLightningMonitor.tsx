...
import { useMapStore, type VolcanicLightningState } from '@/lib/map-store'
import { Bolt as BoltIcon2, X, MapPin, TrendingUp, TrendingDown, Minus, Activity } from 'lucide-react'

const SAMPLE_LOCATIONS = [
  { name: 'Sakurajima, Japan', value: 45, unit: 'flashes/min', status: 'critical', trend: 'up', lat: 31.6, lng: 130.7, description: 'Persistent volcanic lightning during frequent Vulcanian eruptions' },
  { name: 'Eyjafjallajökull, Iceland', value: 28, unit: 'flashes/min', status: 'warning', trend: 'stable', lat: 63.6, lng: -19.6, description: 'Ash-cloud lightning during 2010 eruption disrupted air traffic' },
  { name: 'Taal, Philippines', value: 35, unit: 'flashes/min', status: 'critical', trend: 'up', lat: 14, lng: 121, description: 'Phreatomagmatic eruptions producing intense lightning' },
  { name: 'Redoubt, Alaska', value: 20, unit: 'flashes/min', status: 'warning', trend: 'down', lat: 60.5, lng: -152.7, description: 'Dome-collapse eruptions with ash-cloud lightning' },
]
...
export function VolcanicLightningMonitor() {
  const data = useMapStore((s) => s.volcanicLightning.data)
  const setData = useMapStore((s) => s.setVolcanicLightning)

  useEffect(() => {
    if (!data) {
      setData({
        data: {
          name: 'Global Volcanic Lightning Monitor',
          value: 32,
          unit: 'flashes/min',
          status: 'warning',
          trend: 'up',
          lat: 31.6,
          lng: 130.7,
          description: 'Monitoring volcanic lightning activity during explosive eruptions',
          ...
          <CardHeader className="pb-3 border-b border-yellow-800/30">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2 text-yellow-100">
                <BoltIcon2 className="h-4 w-4 text-yellow-400" />
                Volcanic Lightning Monitor
              </CardTitle>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-yellow-700/30 bg-yellow-900/30 p-2 text-center">
                    <div className="text-[10px] text-yellow-400">Avg Flash Rate</div>
                    <div className="text-lg font-semibold text-yellow-200">{data.value.toFixed(0)}</div>
