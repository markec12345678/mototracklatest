...
import { useMapStore, type CoastalWetlandState } from '@/lib/map-store'
import { Waves as WavesIcon12, X, MapPin, TrendingUp, TrendingDown, Minus, Activity } from 'lucide-react'

const SAMPLE_LOCATIONS = [
  { name: 'Louisiana Delta', value: 4800, unit: 'km²', status: 'critical', trend: 'down', lat: 29.5, lng: -91.0, description: 'Rapid wetland loss from subsidence, erosion, and saltwater intrusion' },
  { name: 'Sundarbans', value: 6200, unit: 'km²', status: 'warning', trend: 'down', lat: 22.0, lng: 89.0, description: 'Mangrove wetland under threat from sea level rise and cyclone damage' },
  { name: 'Wadden Sea', value: 3800, unit: 'km²', status: 'warning', trend: 'stable', lat: 53.0, lng: 8.0, description: 'Tidal flat system under pressure from sea level rise and tourism' },
  { name: 'Everglades', value: 5100, unit: 'km²', status: 'normal', trend: 'up', lat: 25.3, lng: -81.0, description: 'Restoration efforts stabilizing wetland area after decades of drainage' },
]
...
export function CoastalWetlandMonitor() {
  const data = useMapStore((s) => s.coastalWetland.data)
  const setData = useMapStore((s) => s.setCoastalWetland)

  useEffect(() => {
    if (!data) {
      setData({
        data: {
          name: 'Global Coastal Wetland Monitor',
          value: 4975.0,
          unit: 'wetland area (km²)',
          status: 'warning',
          trend: 'down',
          lat: 29.5,
          lng: -91.0,
          description: 'Monitoring coastal wetland area changes at critical deltas and estuaries worldwide',
          ...
          <CardHeader className="pb-3 border-b border-lime-800/30">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2 text-lime-100">
                <WavesIcon12 className="h-4 w-4 text-lime-400" />
                Coastal Wetland Monitor
              </CardTitle>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-lime-700/30 bg-lime-900/30 p-2 text-center">
                    <div className="text-[10px] text-lime-400">Wetland Area</div>
