...
import { MapPin, Layers, Route, ZoomIn } from 'lucide-react'
...
export function MapStatsPanel() {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-1.5 bg-background/90 backdrop-blur-sm rounded-xl border shadow-md">
      {stats.map((stat, i) => (
        <div key={stat.label} className="flex items-center gap-1.5">
          ...
          {i < stats.length - 1 && (
            <span className="w-px h-3 bg-border ml-1.5" />
          )}
        </div>
      ))}
    </div>
