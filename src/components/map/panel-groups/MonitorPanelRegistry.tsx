...
export function MonitorPanelRegistry() {
  ...
  const bookstoreRetailChain = useMapStore((s) => s.bookstoreRetailChain)
  const giftSpecialtyShop = useMapStore((s) => s.giftSpecialtyShop)
  const wholesaleClubWarehouse = useMapStore((s) => s.wholesaleClubWarehouse)
  const partySupplyStore = useMapStore((s) => s.partySupplyStore)
  const pharmacyDrugStore = useMapStore((s) => s.pharmacyDrugStore)
  ...
  const wineSpiritsShop = useMapStore((s) => s.wineSpiritsShop)
  const coffeeRoasterCafe = useMapStore((s) => s.coffeeRoasterCafe)
  const teaSpiceMerchant = useMapStore((s) => s.teaSpiceMerchant)
  const chocolateConfectionery = useMapStore((s) => s.chocolateConfectionery)
  const donutBakeryShop = useMapStore((s) => s.donutBakeryShop)
  ...
  const beverageDistributionCenter = useMapStore((s) => s.beverageDistributionCenter)
  const vendingMachineOperator = useMapStore((s) => s.vendingMachineOperator)
  const foodTruckCommissary = useMapStore((s) => s.foodTruckCommissary)
  const cateringEventHall = useMapStore((s) => s.cateringEventHall)
  const cookingSchoolCulinaryInstitute = useMapStore((s) => s.cookingSchoolCulinaryInstitute)
  const foodBankPantry = useMapStore((s) => s.foodBankPantry)
  const soupKitchenShelter = useMapStore((s) => s.soupKitchenShelter)
  const schoolCafeteriaOperator = useMapStore((s) => s.schoolCafeteriaOperator)
  // Task 165: Institutional & Hospitality Dining monitors
  const hospitalFoodService = useMapStore((s) => s.hospitalFoodService)
  const corporateDiningService = useMapStore((s) => s.corporateDiningService)
  const hotelRestaurantSupply = useMapStore((s) => s.hotelRestaurantSupply)
  const barNightclubSupply = useMapStore((s) => s.barNightclubSupply)
  const breweryTaproom = useMapStore((s) => s.breweryTaproom)
  const distilleryTastingRoom = useMapStore((s) => s.distilleryTastingRoom)
  const wineryVineyard = useMapStore((s) => s.wineryVineyard)
  const weddingEventVenue = useMapStore((s) => s.weddingEventVenue)
  // Task 166: Entertainment & Travel Venue monitors
  const conferenceConventionCenter = useMapStore((s) => s.conferenceConventionCenter)
  const concertMusicHall = useMapStore((s) => s.concertMusicHall)
  const stadiumArenaConcession = useMapStore((s) => s.stadiumArenaConcession)
  const movieTheaterConcession = useMapStore((s) => s.movieTheaterConcession)
  const museumCafeGift = useMapStore((s) => s.museumCafeGift)
  const themeParkRestaurant = useMapStore((s) => s.themeParkRestaurant)
  const cruiseShipGalley = useMapStore((s) => s.cruiseShipGalley)
  const airportFoodCourt = useMapStore((s) => s.airportFoodCourt)
  // Task 167: Specialty Hospitality & Attraction Venue monitors
  const hospitalCafeGiftShop = useMapStore((s) => s.hospitalCafeGiftShop)
  const hotelRoomService = useMapStore((s) => s.hotelRoomService)
  const casinoRestaurant = useMapStore((s) => s.casinoRestaurant)
  const stadiumPremiumSuite = useMapStore((s) => s.stadiumPremiumSuite)
  const aquariumCafe = useMapStore((s) => s.aquariumCafe)
  const zooConcession = useMapStore((s) => s.zooConcession)
  const botanicalGardenCafe = useMapStore((s) => s.botanicalGardenCafe)
  const nationalParkLodge = useMapStore((s) => s.nationalParkLodge)
  // Task 168: Travel & Recreation Venue monitors
  const beachResortRestaurant = useMapStore((s) => s.beachResortRestaurant)
  const mountainSkiLodgeRestaurant = useMapStore((s) => s.mountainSkiLodgeRestaurant)
  const golfCountryClubRestaurant = useMapStore((s) => s.golfCountryClubRestaurant)
  const marinaYachtClub = useMapStore((s) => s.marinaYachtClub)
  const casinoHotelBuffet = useMapStore((s) => s.casinoHotelBuffet)
  ...
  const driveInTheaterConcession = useMapStore((s) => s.driveInTheaterConcession)
  const miniGolfSnackBar = useMapStore((s) => s.miniGolfSnackBar)
  const goKartTrackConcession = useMapStore((s) => s.goKartTrackConcession)
  const rollerRinkSnackBar = useMapStore((s) => s.rollerRinkSnackBar)
  const iceRinkCafe = useMapStore((s) => s.iceRinkCafe)
  const paintballParkCafe = useMapStore((s) => s.paintballParkCafe)
  const zipLineTourCafe = useMapStore((s) => s.zipLineTourCafe)
  const bungeeJumpSiteCafe = useMapStore((s) => s.bungeeJumpSiteCafe)
  // Task 172: Action Sports & Adventure Lounge monitors
  const trampolineParkCafe = useMapStore((s) => s.trampolineParkCafe)
  const laserTagArenaSnackBar = useMapStore((s) => s.laserTagArenaSnackBar)
  const escapeRoomLounge = useMapStore((s) => s.escapeRoomLounge)
  const axeThrowingVenueBar = useMapStore((s) => s.axeThrowingVenueBar)
  const climbingGymCafe = useMapStore((s) => s.climbingGymCafe)
  const skateParkSnackBar = useMapStore((s) => s.skateParkSnackBar)
  const discGolfCourseConcession = useMapStore((s) => s.discGolfCourseConcession)
  const bmxTrackConcession = useMapStore((s) => s.bmxTrackConcession)
  // Task 173: Paddle & Wind Water Sports Venue monitors
  const rollerDerbyVenueBar = useMapStore((s) => s.rollerDerbyVenueBar)
  const indoorSkydivingLounge = useMapStore((s) => s.indoorSkydivingLounge)
  const surfSchoolCafe = useMapStore((s) => s.surfSchoolCafe)
  const kiteboardingBeachBar = useMapStore((s) => s.kiteboardingBeachBar)
  const windsurfingShoreCafe = useMapStore((s) => s.windsurfingShoreCafe)
  const kayakTourSnackBar = useMapStore((s) => s.kayakTourSnackBar)
  const canoeRentalCafe = useMapStore((s) => s.canoeRentalCafe)
  const paddleboardRentalCafe = useMapStore((s) => s.paddleboardRentalCafe)
  // Task 174: Water Concession & Wellness Retreat
  const whitewaterRaftingConcession = useMapStore((s) => s.whitewaterRaftingConcession)
  const jetSkiRentalSnackBar = useMapStore((s) => s.jetSkiRentalSnackBar)
  const sailingClubBar = useMapStore((s) => s.sailingClubBar)
  const marinaRestaurant = useMapStore((s) => s.marinaRestaurant)
  const houseboatRentalCafe = useMapStore((s) => s.houseboatRentalCafe)
  ...
  return (
    <>
      ...
      {bookstoreRetailChain.open && (
        <LazyPanel importFn={() => import('@/components/map/BookstoreRetailChainMonitor')} exportName="BookstoreRetailChainMonitor" shouldLoad={bookstoreRetailChain.open} />
      )}
      {giftSpecialtyShop.open && (
        <LazyPanel importFn={() => import('@/components/map/GiftSpecialtyShopMonitor')} exportName="GiftSpecialtyShopMonitor" shouldLoad={giftSpecialtyShop.open} />
      )}
      {wholesaleClubWarehouse.open && (
        <LazyPanel importFn={() => import('@/components/map/WholesaleClubWarehouseMonitor')} exportName="WholesaleClubWarehouseMonitor" shouldLoad={wholesaleClubWarehouse.open} />
      )}
      {partySupplyStore.open && (
        <LazyPanel importFn={() => import('@/components/map/PartySupplyStoreMonitor')} exportName="PartySupplyStoreMonitor" shouldLoad={partySupplyStore.open} />
      )}
      {pharmacyDrugStore.open && (
        <LazyPanel importFn={() => import('@/components/map/PharmacyDrugStoreMonitor')} exportName="PharmacyDrugStoreMonitor" shouldLoad={pharmacyDrugStore.open} />
      )}
      {buildingSupplyCenter.open && (
        <LazyPanel importFn={() => import('@/components/map/BuildingSupplyCenterMonitor')} exportName="BuildingSupplyCenterMonitor" shouldLoad={buildingSupplyCenter.open} />
      )}
      {gardenCenterFlorist.open && (
        <LazyPanel importFn={() => import('@/components/map/GardenCenterFloristMonitor')} exportName="GardenCenterFloristMonitor" shouldLoad={gardenCenterFlorist.open} />
      )}
      ...
      {wineSpiritsShop.open && (
        <LazyPanel importFn={() => import('@/components/map/WineSpiritsShopMonitor')} exportName="WineSpiritsShopMonitor" shouldLoad={wineSpiritsShop.open} />
      )}
      {coffeeRoasterCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/CoffeeRoasterCafeMonitor')} exportName="CoffeeRoasterCafeMonitor" shouldLoad={coffeeRoasterCafe.open} />
      )}
      {teaSpiceMerchant.open && (
        <LazyPanel importFn={() => import('@/components/map/TeaSpiceMerchantMonitor')} exportName="TeaSpiceMerchantMonitor" shouldLoad={teaSpiceMerchant.open} />
      )}
      {chocolateConfectionery.open && (
        <LazyPanel importFn={() => import('@/components/map/ChocolateConfectioneryMonitor')} exportName="ChocolateConfectioneryMonitor" shouldLoad={chocolateConfectionery.open} />
      )}
      {donutBakeryShop.open && (
        <LazyPanel importFn={() => import('@/components/map/DonutBakeryShopMonitor')} exportName="DonutBakeryShopMonitor" shouldLoad={donutBakeryShop.open} />
      )}
      {iceCreamDessertShop.open && (
        <LazyPanel importFn={() => import('@/components/map/IceCreamDessertShopMonitor')} exportName="IceCreamDessertShopMonitor" shouldLoad={iceCreamDessertShop.open} />
      )}
      {bagelDeliShop.open && (
        <LazyPanel importFn={() => import('@/components/map/BagelDeliShopMonitor')} exportName="BagelDeliShopMonitor" shouldLoad={bagelDeliShop.open} />
      )}
      ...
      {beverageDistributionCenter.open && (
        <LazyPanel importFn={() => import('@/components/map/BeverageDistributionCenterMonitor')} exportName="BeverageDistributionCenterMonitor" shouldLoad={beverageDistributionCenter.open} />
      )}
      {vendingMachineOperator.open && (
        <LazyPanel importFn={() => import('@/components/map/VendingMachineOperatorMonitor')} exportName="VendingMachineOperatorMonitor" shouldLoad={vendingMachineOperator.open} />
      )}
      {foodTruckCommissary.open && (
        <LazyPanel importFn={() => import('@/components/map/FoodTruckCommissaryMonitor')} exportName="FoodTruckCommissaryMonitor" shouldLoad={foodTruckCommissary.open} />
      )}
      {cateringEventHall.open && (
        <LazyPanel importFn={() => import('@/components/map/CateringEventHallMonitor')} exportName="CateringEventHallMonitor" shouldLoad={cateringEventHall.open} />
      )}
      {cookingSchoolCulinaryInstitute.open && (
        <LazyPanel importFn={() => import('@/components/map/CookingSchoolCulinaryInstituteMonitor')} exportName="CookingSchoolCulinaryInstituteMonitor" shouldLoad={cookingSchoolCulinaryInstitute.open} />
      )}
      {foodBankPantry.open && (
        <LazyPanel importFn={() => import('@/components/map/FoodBankPantryMonitor')} exportName="FoodBankPantryMonitor" shouldLoad={foodBankPantry.open} />
      )}
      {soupKitchenShelter.open && (
        <LazyPanel importFn={() => import('@/components/map/SoupKitchenShelterMonitor')} exportName="SoupKitchenShelterMonitor" shouldLoad={soupKitchenShelter.open} />
      )}
      {schoolCafeteriaOperator.open && (
        <LazyPanel importFn={() => import('@/components/map/SchoolCafeteriaOperatorMonitor')} exportName="SchoolCafeteriaOperatorMonitor" shouldLoad={schoolCafeteriaOperator.open} />
      )}
      {hospitalFoodService.open && (
        <LazyPanel importFn={() => import('@/components/map/HospitalFoodServiceMonitor')} exportName="HospitalFoodServiceMonitor" shouldLoad={hospitalFoodService.open} />
      )}
      {corporateDiningService.open && (
        <LazyPanel importFn={() => import('@/components/map/CorporateDiningServiceMonitor')} exportName="CorporateDiningServiceMonitor" shouldLoad={corporateDiningService.open} />
      )}
      {hotelRestaurantSupply.open && (
        <LazyPanel importFn={() => import('@/components/map/HotelRestaurantSupplyMonitor')} exportName="HotelRestaurantSupplyMonitor" shouldLoad={hotelRestaurantSupply.open} />
      )}
      {barNightclubSupply.open && (
        <LazyPanel importFn={() => import('@/components/map/BarNightclubSupplyMonitor')} exportName="BarNightclubSupplyMonitor" shouldLoad={barNightclubSupply.open} />
      )}
      {breweryTaproom.open && (
        <LazyPanel importFn={() => import('@/components/map/BreweryTaproomMonitor')} exportName="BreweryTaproomMonitor" shouldLoad={breweryTaproom.open} />
      )}
      {distilleryTastingRoom.open && (
        <LazyPanel importFn={() => import('@/components/map/DistilleryTastingRoomMonitor')} exportName="DistilleryTastingRoomMonitor" shouldLoad={distilleryTastingRoom.open} />
      )}
      {wineryVineyard.open && (
        <LazyPanel importFn={() => import('@/components/map/WineryVineyardMonitor')} exportName="WineryVineyardMonitor" shouldLoad={wineryVineyard.open} />
      )}
      {weddingEventVenue.open && (
        <LazyPanel importFn={() => import('@/components/map/WeddingEventVenueMonitor')} exportName="WeddingEventVenueMonitor" shouldLoad={weddingEventVenue.open} />
      )}
      {conferenceConventionCenter.open && (
        <LazyPanel importFn={() => import('@/components/map/ConferenceConventionCenterMonitor')} exportName="ConferenceConventionCenterMonitor" shouldLoad={conferenceConventionCenter.open} />
      )}
      {concertMusicHall.open && (
        <LazyPanel importFn={() => import('@/components/map/ConcertMusicHallMonitor')} exportName="ConcertMusicHallMonitor" shouldLoad={concertMusicHall.open} />
      )}
      {stadiumArenaConcession.open && (
        <LazyPanel importFn={() => import('@/components/map/StadiumArenaConcessionMonitor')} exportName="StadiumArenaConcessionMonitor" shouldLoad={stadiumArenaConcession.open} />
      )}
      {movieTheaterConcession.open && (
        <LazyPanel importFn={() => import('@/components/map/MovieTheaterConcessionMonitor')} exportName="MovieTheaterConcessionMonitor" shouldLoad={movieTheaterConcession.open} />
      )}
      {museumCafeGift.open && (
        <LazyPanel importFn={() => import('@/components/map/MuseumCafeGiftMonitor')} exportName="MuseumCafeGiftMonitor" shouldLoad={museumCafeGift.open} />
      )}
      {themeParkRestaurant.open && (
        <LazyPanel importFn={() => import('@/components/map/ThemeParkRestaurantMonitor')} exportName="ThemeParkRestaurantMonitor" shouldLoad={themeParkRestaurant.open} />
      )}
      {cruiseShipGalley.open && (
        <LazyPanel importFn={() => import('@/components/map/CruiseShipGalleyMonitor')} exportName="CruiseShipGalleyMonitor" shouldLoad={cruiseShipGalley.open} />
      )}
      {airportFoodCourt.open && (
        <LazyPanel importFn={() => import('@/components/map/AirportFoodCourtMonitor')} exportName="AirportFoodCourtMonitor" shouldLoad={airportFoodCourt.open} />
      )}
      {hospitalCafeGiftShop.open && (
        <LazyPanel importFn={() => import('@/components/map/HospitalCafeGiftShopMonitor')} exportName="HospitalCafeGiftShopMonitor" shouldLoad={hospitalCafeGiftShop.open} />
      )}
      {hotelRoomService.open && (
        <LazyPanel importFn={() => import('@/components/map/HotelRoomServiceMonitor')} exportName="HotelRoomServiceMonitor" shouldLoad={hotelRoomService.open} />
      )}
      {casinoRestaurant.open && (
        <LazyPanel importFn={() => import('@/components/map/CasinoRestaurantMonitor')} exportName="CasinoRestaurantMonitor" shouldLoad={casinoRestaurant.open} />
      )}
      {stadiumPremiumSuite.open && (
        <LazyPanel importFn={() => import('@/components/map/StadiumPremiumSuiteMonitor')} exportName="StadiumPremiumSuiteMonitor" shouldLoad={stadiumPremiumSuite.open} />
      )}
      {aquariumCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/AquariumCafeMonitor')} exportName="AquariumCafeMonitor" shouldLoad={aquariumCafe.open} />
      )}
      {zooConcession.open && (
        <LazyPanel importFn={() => import('@/components/map/ZooConcessionMonitor')} exportName="ZooConcessionMonitor" shouldLoad={zooConcession.open} />
      )}
      {botanicalGardenCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/BotanicalGardenCafeMonitor')} exportName="BotanicalGardenCafeMonitor" shouldLoad={botanicalGardenCafe.open} />
      )}
      {nationalParkLodge.open && (
        <LazyPanel importFn={() => import('@/components/map/NationalParkLodgeMonitor')} exportName="NationalParkLodgeMonitor" shouldLoad={nationalParkLodge.open} />
      )}
      {/* Task 168: Travel & Recreation Venue monitors */}
      {beachResortRestaurant.open && (
        <LazyPanel importFn={() => import('@/components/map/BeachResortRestaurantMonitor')} exportName="BeachResortRestaurantMonitor" shouldLoad={beachResortRestaurant.open} />
      )}
      {mountainSkiLodgeRestaurant.open && (
        <LazyPanel importFn={() => import('@/components/map/MountainSkiLodgeRestaurantMonitor')} exportName="MountainSkiLodgeRestaurantMonitor" shouldLoad={mountainSkiLodgeRestaurant.open} />
      )}
      {golfCountryClubRestaurant.open && (
        <LazyPanel importFn={() => import('@/components/map/GolfCountryClubRestaurantMonitor')} exportName="GolfCountryClubRestaurantMonitor" shouldLoad={golfCountryClubRestaurant.open} />
      )}
      {marinaYachtClub.open && (
        <LazyPanel importFn={() => import('@/components/map/MarinaYachtClubMonitor')} exportName="MarinaYachtClubMonitor" shouldLoad={marinaYachtClub.open} />
      )}
      {casinoHotelBuffet.open && (
        <LazyPanel importFn={() => import('@/components/map/CasinoHotelBuffetMonitor')} exportName="CasinoHotelBuffetMonitor" shouldLoad={casinoHotelBuffet.open} />
      )}
      {highwayRestArea.open && (
        <LazyPanel importFn={() => import('@/components/map/HighwayRestAreaMonitor')} exportName="HighwayRestAreaMonitor" shouldLoad={highwayRestArea.open} />
      )}
      {trainStationDining.open && (
        <LazyPanel importFn={() => import('@/components/map/TrainStationDiningMonitor')} exportName="TrainStationDiningMonitor" shouldLoad={trainStationDining.open} />
      )}
      ...
      {driveInTheaterConcession.open && (
        <LazyPanel importFn={() => import('@/components/map/DriveInTheaterConcessionMonitor')} exportName="DriveInTheaterConcessionMonitor" shouldLoad={driveInTheaterConcession.open} />
      )}
      {miniGolfSnackBar.open && (
        <LazyPanel importFn={() => import('@/components/map/MiniGolfSnackBarMonitor')} exportName="MiniGolfSnackBarMonitor" shouldLoad={miniGolfSnackBar.open} />
      )}
      {goKartTrackConcession.open && (
        <LazyPanel importFn={() => import('@/components/map/GoKartTrackConcessionMonitor')} exportName="GoKartTrackConcessionMonitor" shouldLoad={goKartTrackConcession.open} />
      )}
      {rollerRinkSnackBar.open && (
        <LazyPanel importFn={() => import('@/components/map/RollerRinkSnackBarMonitor')} exportName="RollerRinkSnackBarMonitor" shouldLoad={rollerRinkSnackBar.open} />
      )}
      {iceRinkCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/IceRinkCafeMonitor')} exportName="IceRinkCafeMonitor" shouldLoad={iceRinkCafe.open} />
      )}
      {paintballParkCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/PaintballParkCafeMonitor')} exportName="PaintballParkCafeMonitor" shouldLoad={paintballParkCafe.open} />
      )}
      {zipLineTourCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/ZipLineTourCafeMonitor')} exportName="ZipLineTourCafeMonitor" shouldLoad={zipLineTourCafe.open} />
      )}
      {bungeeJumpSiteCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/BungeeJumpSiteCafeMonitor')} exportName="BungeeJumpSiteCafeMonitor" shouldLoad={bungeeJumpSiteCafe.open} />
      )}
      {trampolineParkCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/TrampolineParkCafeMonitor')} exportName="TrampolineParkCafeMonitor" shouldLoad={trampolineParkCafe.open} />
      )}
      {laserTagArenaSnackBar.open && (
        <LazyPanel importFn={() => import('@/components/map/LaserTagArenaSnackBarMonitor')} exportName="LaserTagArenaSnackBarMonitor" shouldLoad={laserTagArenaSnackBar.open} />
      )}
      {escapeRoomLounge.open && (
        <LazyPanel importFn={() => import('@/components/map/EscapeRoomLoungeMonitor')} exportName="EscapeRoomLoungeMonitor" shouldLoad={escapeRoomLounge.open} />
      )}
      {axeThrowingVenueBar.open && (
        <LazyPanel importFn={() => import('@/components/map/AxeThrowingVenueBarMonitor')} exportName="AxeThrowingVenueBarMonitor" shouldLoad={axeThrowingVenueBar.open} />
      )}
      {climbingGymCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/ClimbingGymCafeMonitor')} exportName="ClimbingGymCafeMonitor" shouldLoad={climbingGymCafe.open} />
      )}
      {skateParkSnackBar.open && (
        <LazyPanel importFn={() => import('@/components/map/SkateParkSnackBarMonitor')} exportName="SkateParkSnackBarMonitor" shouldLoad={skateParkSnackBar.open} />
      )}
      {discGolfCourseConcession.open && (
        <LazyPanel importFn={() => import('@/components/map/DiscGolfCourseConcessionMonitor')} exportName="DiscGolfCourseConcessionMonitor" shouldLoad={discGolfCourseConcession.open} />
      )}
      {bmxTrackConcession.open && (
        <LazyPanel importFn={() => import('@/components/map/BMXTrackConcessionMonitor')} exportName="BMXTrackConcessionMonitor" shouldLoad={bmxTrackConcession.open} />
      )}
      {rollerDerbyVenueBar.open && (
        <LazyPanel importFn={() => import('@/components/map/RollerDerbyVenueBarMonitor')} exportName="RollerDerbyVenueBarMonitor" shouldLoad={rollerDerbyVenueBar.open} />
      )}
      {indoorSkydivingLounge.open && (
        <LazyPanel importFn={() => import('@/components/map/IndoorSkydivingLoungeMonitor')} exportName="IndoorSkydivingLoungeMonitor" shouldLoad={indoorSkydivingLounge.open} />
      )}
      {surfSchoolCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/SurfSchoolCafeMonitor')} exportName="SurfSchoolCafeMonitor" shouldLoad={surfSchoolCafe.open} />
      )}
      {kiteboardingBeachBar.open && (
        <LazyPanel importFn={() => import('@/components/map/KiteboardingBeachBarMonitor')} exportName="KiteboardingBeachBarMonitor" shouldLoad={kiteboardingBeachBar.open} />
      )}
      {windsurfingShoreCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/WindsurfingShoreCafeMonitor')} exportName="WindsurfingShoreCafeMonitor" shouldLoad={windsurfingShoreCafe.open} />
      )}
      {kayakTourSnackBar.open && (
        <LazyPanel importFn={() => import('@/components/map/KayakTourSnackBarMonitor')} exportName="KayakTourSnackBarMonitor" shouldLoad={kayakTourSnackBar.open} />
      )}
      {canoeRentalCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/CanoeRentalCafeMonitor')} exportName="CanoeRentalCafeMonitor" shouldLoad={canoeRentalCafe.open} />
      )}
      {paddleboardRentalCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/PaddleboardRentalCafeMonitor')} exportName="PaddleboardRentalCafeMonitor" shouldLoad={paddleboardRentalCafe.open} />
      )}
      {whitewaterRaftingConcession.open && (
        <LazyPanel importFn={() => import('@/components/map/WhitewaterRaftingConcessionMonitor')} exportName="WhitewaterRaftingConcessionMonitor" shouldLoad={whitewaterRaftingConcession.open} />
      )}
      {jetSkiRentalSnackBar.open && (
        <LazyPanel importFn={() => import('@/components/map/JetSkiRentalSnackBarMonitor')} exportName="JetSkiRentalSnackBarMonitor" shouldLoad={jetSkiRentalSnackBar.open} />
      )}
      {sailingClubBar.open && (
        <LazyPanel importFn={() => import('@/components/map/SailingClubBarMonitor')} exportName="SailingClubBarMonitor" shouldLoad={sailingClubBar.open} />
      )}
      {marinaRestaurant.open && (
        <LazyPanel importFn={() => import('@/components/map/MarinaRestaurantMonitor')} exportName="MarinaRestaurantMonitor" shouldLoad={marinaRestaurant.open} />
      )}
      {houseboatRentalCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/HouseboatRentalCafeMonitor')} exportName="HouseboatRentalCafeMonitor" shouldLoad={houseboatRentalCafe.open} />
      )}
      {floatSpaLounge.open && (
        <LazyPanel importFn={() => import('@/components/map/FloatSpaLoungeMonitor')} exportName="FloatSpaLoungeMonitor" shouldLoad={floatSpaLounge.open} />
      )}
      {saltCaveRelaxationCafe.open && (
        <LazyPanel importFn={() => import('@/components/map/SaltCaveRelaxationCafeMonitor')} exportName="SaltCaveRelaxationCafeMonitor" shouldLoad={saltCaveRelaxationCafe.open} />
      )}
      {daySpaCafe.open && (
