...
import { useMapStore, type GeomagneticStormState } from '@/lib/map-store'
import { Zap as ZapIcon4, X, MapPin, TrendingUp, TrendingDown, Minus, Activity } from 'lucide-react'

const SAMPLE_LOCATIONS = [
  { name: 'Auroral Zone', value: 7, unit: 'Kp index', status: 'critical', trend: 'up', lat: 65.0, lng: 25.0, description: 'High-latitude auroral zone with frequent substorm activity and geomagnetic disturbances' },
  { name: 'South Atlantic Anomaly', value: 5, unit: 'Kp index', status: 'warning', trend: 'up', lat: -30.0, lng: -40.0, description: 'Weakened magnetic field region, increased radiation exposure for satellites' },
  { name: 'North Pole', value: 8, unit: 'Kp index', status: 'critical', trend: 'stable', lat: 90.0, lng: 0.0, description: 'Polar cap region with direct solar wind particle access and strong disturbances' },
  { name: 'High Latitude Siberia', value: 6, unit: 'Kp index', status: 'warning', trend: 'down', lat: 70.0, lng: 100.0, description: 'Siberian high-latitude station monitoring auroral electrojet disturbances' },
]
...
export function GeomagneticStormMonitor() {
  const data = useMapStore((s) => s.geomagneticStorm.data)
  const setData = useMapStore((s) => s.setGeomagneticStorm)

  useEffect(() => {
    if (!data) {
      setData({
        data: {
          name: 'Global Geomagnetic Storm Monitor',
          value: 6.5,
          unit: 'avg Kp index',
          status: 'warning',
          trend: 'up',
          lat: 65.0,
          lng: 25.0,
          description: 'Monitoring geomagnetic storm intensity and Kp index at key observatory locations worldwide',
          ...
          <CardHeader className="pb-3 border-b border-purple-800/30">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2 text-purple-100">
                <ZapIcon4 className="h-4 w-4 text-purple-400" />
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-purple-700/30 bg-purple-900/30 p-2 text-center">
                    <div className="text-[10px] text-purple-400">Avg Kp Index</div>
