...
import { useMapStore, type ThermokarstErosionState } from '@/lib/map-store'
import { Thermometer as ThermometerIcon8, X, MapPin, TrendingUp, TrendingDown, Minus, Activity } from 'lucide-react'

const SAMPLE_LOCATIONS = [
  { name: 'Yedoma Siberia', value: 4.5, unit: 'subsidence rate (cm/yr)', status: 'critical', trend: 'up', lat: 68.0, lng: 135.0, description: 'Ice-rich permafrost thaw causing rapid thermokarst lake expansion and ground collapse' },
  { name: 'Mackenzie Delta', value: 3.2, unit: 'subsidence rate (cm/yr)', status: 'warning', trend: 'up', lat: 69.0, lng: -135.0, description: 'Active layer deepening and thermo-erosion along Arctic coast and delta channels' },
  { name: 'Lena Delta', value: 2.8, unit: 'subsidence rate (cm/yr)', status: 'warning', trend: 'stable', lat: 73.0, lng: 130.0, description: 'Permafrost degradation with polygonal ground collapse and lake drainage events' },
  { name: 'North Slope Alaska', value: 1.5, unit: 'subsidence rate (cm/yr)', status: 'normal', trend: 'down', lat: 70.0, lng: -155.0, description: 'Coastal plain thermokarst monitoring with variable subsidence near Prudhoe Bay' },
]
...
export function ThermokarstErosionMonitor() {
  const data = useMapStore((s) => s.thermokarstErosion.data)
  const setData = useMapStore((s) => s.setThermokarstErosion)

  useEffect(() => {
    if (!data) {
      setData({
        data: {
          name: 'Global Thermokarst Erosion Monitor',
          value: 3.0,
          unit: 'avg subsidence rate (cm/yr)',
          status: 'warning',
          trend: 'up',
          lat: 68.0,
          lng: 135.0,
          description: 'Monitoring thermokarst erosion and permafrost subsidence in Arctic and sub-Arctic regions',
          ...
          <CardHeader className="pb-3 border-b border-slate-800/30">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2 text-slate-100">
                <ThermometerIcon8 className="h-4 w-4 text-slate-400" />
                Thermokarst Erosion Monitor
              </CardTitle>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-slate-700/30 bg-slate-900/30 p-2 text-center">
                    <div className="text-[10px] text-slate-400">Avg Subsidence Rate</div>
